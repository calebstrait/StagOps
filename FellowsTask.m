...
eboxcolor = [100 255 0]; %Background color
...
while(continuing);
    ...
    if(step == 2)
        ...
        Screen('FillOval', window, eboxcolor, [(targX-radius)-hD ((targY-radius)) (targX+radius)-hD (targY+radius)]);
        Screen('FillOval', window, eboxcolor, [(targX-radius)+hD ((targY-radius)) (targX+radius)+hD (targY+radius)]);
        if(leftatt > 0), Screen('FillRect', window, color1, [x1min y1min x1max (y1min + (leftwait * height))]);end
        if(rightatt > 0), Screen('FillRect', window, color1, [x2min y2min x2max (y2min + (rightwait * height))]);end
